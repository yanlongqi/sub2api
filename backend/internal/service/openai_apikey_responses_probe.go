...
func (s *AccountTestService) ProbeOpenAIAPIKeyResponsesSupport(ctx context.Context, accountID int64) {
	...
	if account.IsCNProvider() {
		...
		// 不存在通用 /v1/responses 端点。直接落标 false 走 Chat Completions 直转，跳过网络探测。
		// 例外：deepseek 与智谱（Coding Plan）的固定 responses 和 adaptive 账号使用
		// 官方原生 /responses 端点，落标 force_responses；其余协议显式重置为 auto，
		// 避免切换后残留强制模式。
		if account.GetAPIProtocol() == APIProtocolResponses ||
			((account.Platform == PlatformDeepseek || account.Platform == PlatformZhipu) && account.IsAdaptiveAPIProtocol()) {
