export default {
    accounts: {
      ...
      upstreamBilling: {
        ...
        rateFactor: 'Upstream rate factor',
        ...
        rateFactorLabel: 'Upstream rate factor: {value}x',
        schedulingRateLabel: 'Scheduling rate: {value}x',
      ...
      quotaSyncUpstreamToggle: 'Sync Upstream Quota',
      quotaSyncUpstreamToggleHint: 'When enabled, periodically calls the upstream sub2api /v1/usage endpoint using this account\'s proxy and API key to sync subscription or balance data into the quota display (refresh cadence matches OpenAI OAuth).',
      quotaSyncUpstreamActiveHint: 'Upstream sync is on: the total quota is maintained by periodic upstream sync, no manual entry needed. Subscription data is shown via the subscription structure; balance data is treated as total quota on first sighting or when it increases, and as remaining quota when it decreases.',
      quotaSyncVolcengineHint: 'Volcengine Ark upstream detected: quota is queried via the control-plane OpenAPI (GetAFPUsage / GetCodingPlanUsage). Fill in the Volcengine account AccessKey ID / Secret (separate credentials from the inference API key; create them in the Volcengine console under "API Access Key Management").',
      quotaSyncVolcengineAccessKeyId: 'AccessKey ID',
      ...
      quotaMonthlyLimitHint: 'Maintained by upstream subscription sync, rolling 30-day window.',
      ...
      homepageUrlHint: 'If set, clicking the account name in the list opens this URL; otherwise the Base URL origin is used',
