export default {
    accounts: {
      ...
      upstreamBilling: {
        ...
        rateFactorLabel: '上游费率倍率：{value}x',
        schedulingRateLabel: '调度使用倍率：{value}x',
      ...
      quotaSyncUpstreamToggle: '同步上游配额',
      quotaSyncUpstreamToggleHint: '开启后，使用当前账号的代理与 API Key 定期调用上游 sub2api /v1/usage 接口，自动同步订阅或余额数据并写入配额显示（刷新节奏与 OpenAI OAuth 一致）。',
      quotaSyncUpstreamActiveHint: '已开启同步上游：总配额由上游定期同步维护，无需手动填写。订阅数据采用订阅结构展示；余额数据在首次或增大时作为总配额，减小时作为剩余配额。',
      quotaSyncVolcengineHint: '检测到火山方舟上游：配额查询走控制面 OpenAPI（GetAFPUsage / GetCodingPlanUsage），需填写火山账号的 AccessKey ID / Secret（与推理 API Key 是两套凭据，可在火山引擎控制台「密钥管理」创建）。',
      ...
      quotaMonthlyLimitHint: '由上游订阅同步维护，每 30 天滚动重置。',
      quotaTotalLimit: '总限额',
      ...
      homepageUrlHint: '填写后账号列表中点击账号名将跳转到该地址；留空则使用 Base URL 的站点主页',
